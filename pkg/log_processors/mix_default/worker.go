...
/*
 * Mix-default worker extends the base worker to merge asr event logs and apply custom transformations
 */
...
// NewWorker() creates a new instance of a mix-default worker
...
func (w *Worker) Transform() pt.LogProcessorWorker {
	...
	// Apply transformations to the event log record
	...
	// Cache the transformed ASR record
...
// Write() writes the transformed event log to the data pipeline
func (w *Worker) Write(writer io.Writer) pt.LogProcessorWorker {
	// We may have two event logs to write (e.g. merged asr-callsummary and asr-finalstatus)...
	if w.joinedCallSummary != nil {
		w.writeAdditionalCallSummaryRecord(writer)
	}

	// Write the transformed event log
	w.parent.Write(writer)
	return w
}
...
// writeAdditionalCallSummaryRecord() writes the joined asr-callsummary record to the data pipeline
func (w *Worker) writeAdditionalCallSummaryRecord(writer io.Writer) {
	bytes, _ := json.Marshal(*w.joinedCallSummary)
	err := writer.Write(bytes)
	if err != nil {
		log.Errorf("[worker id: %v] %v", w.parent.ID.String(), err)
	} else {
		log.Debugf("[worker id: %v] log write (joined call summary record): PASSED", w.parent.ID.String())
	}
}
