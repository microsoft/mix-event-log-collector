...
/*
 * opensearch writer implements a log writer that writes event logs to
 * opensearch as the storage provider.
 */
...
var (
	// Default opensearch writer config
...
// Config is the opensearch writer configuration
type Config struct {
	...
	// overlay of the opensearch.Config struct
...
// LogWriter is the opensearch LogWriter implementation
type LogWriter struct {
	...
	reader     iot.Reader
	opensearch wt.Storage
}

//// Public methods

// NewLogWriter() creates a new instance of an opensearch LogWriter
func NewLogWriter(configFile string) wt.LogWriter {
	log.Infof("factory creating new opensearch log writer")
	...
	writer := LogWriter{
		...
		opensearch:       NewStorage(cfg.Writer.Config),
	}
...
func loadConfigFromFile(configFile string) (*LogWriterYaml, error) {
	...
	yml.Writer.Config.Username = getValueFromEnv("ELC_OPENSEARCH_USERNAME", yml.Writer.Config.Username)
	yml.Writer.Config.Password = getValueFromEnv("ELC_OPENSEARCH_PASSWORD", yml.Writer.Config.Password)
	...
	for {
		...
		case record := <-l.records:
			...
			if err == nil {
				...
				log.Debugf("writing to opensearch [index: %v] [docID: %v]: %v", index, docID, string(record))
				_, err = l.opensearch.Write(index, docID, record)
			}
...
func (l *LogWriter) Start() error {
	...
	log.Infof("Starting %v writer. Writing logs to index %v", l.name, l.cfg.IndexPrefix)
	return nil
}
